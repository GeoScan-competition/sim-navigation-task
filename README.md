...
### Отборочный этап
В отборочном этапе используется две карты местности: открытая и закрытая. Открытая доступна вам в симуляторе, на ней вы проверяете своё решение. Зачётная же попытка будет производится организаторами на закрытой карте. Закрытая карта будет схожа с открытой: она может отличатся по размеру, но ширина дорожки и размеры объектов остаются теми же. Закрытая карта не публикуется. Фотографии открытой карты с подписями в файле [POLYGON.md].
